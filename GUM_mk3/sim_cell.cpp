...
void SimCell::fillUnitCell(string POSCAR_file, bool use_poscar) {
	...
	poscar_comp[0] = stoi(split(pos_lines[6], " ")[0]);
	poscar_comp[1] = stoi(split(pos_lines[6], " ")[1]);
	poscar_comp[2] = stoi(split(pos_lines[6], " ")[2]);
	if (use_poscar == false) {
		...
		for (int i = 0; i < pos_list_f.size(); i++) {
			...
			else if (i < poscar_comp[0] + poscar_comp[1]) {
...
void SimCell::initSimCell(string POSCAR_file, vector<float> &dist_list, int _sup_cell[3], vector<int> &_species_numbs, float _cutoff, string _sim_type, string _phase_init, string spin_init, string species_init, bool use_poscar) {
	sim_type = _sim_type;
	phase_init = _phase_init;
	cutoff = _cutoff;
	for (int i = 0; i < _species_numbs.size(); i++) {
